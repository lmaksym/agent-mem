...
export default async function branch({ args, flags }) {
  ...
  writeContextFile(ctxDir, `branches/${name}/purpose.md`, [
    `# Branch: ${name}`,
    "",
  ...
  writeContextFile(ctxDir, `branches/${name}/commits.md`, [
    `# Commits: ${name}`,
    "",
    "Milestone log for this branch.",
    "",
  ].join("\n"));

  writeContextFile(ctxDir, `branches/${name}/trace.md`, [
    `# Trace: ${name}`,
    "",
    "Fine-grained execution log.",
    "",
  ].join("\n"));
