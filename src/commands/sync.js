...
export default async function sync({ args, flags }) {
  ...
  if (!targets.length) {
    ...
    if (!targets.length) {
      ...
      console.log("  agent-context sync --claude     → CLAUDE.md");
      console.log("  agent-context sync --gemini     → GEMINI.md");
      console.log("  agent-context sync --codex      → AGENTS.md");
      console.log("  agent-context sync --cursor     → .cursor/rules/");
      console.log("  agent-context sync --windsurf   → .windsurfrules");
      console.log("  agent-context sync --all        → all of the above");
      return;
    }
  ...
  console.log("Tip: run after 'agent-context commit' to keep IDE rules up to date.");
}
...
function gatherMemorySummary(ctxDir) {
  ...
  const lines = ["## Key Decisions & Patterns", "", "From `.context/memory/` — run `agent-context read <path>` for details:", ""];
...
function buildClaudeMd(systemContent, memorySummary, config) {
  const sections = [
    "# Project Context",
    "",
    "_Auto-generated by agent-context. Edit `.context/system/` files instead._",
    "_Run `agent-context sync --claude` to regenerate._",
    "",
  ];
  ...
  sections.push("");
  sections.push("This project uses `agent-context` for persistent context. Key commands:");
  sections.push("- `agent-context snapshot` — view full context tree");
  sections.push("- `agent-context read <path>` — read specific context file");
  sections.push("- `agent-context commit <msg>` — checkpoint progress");
  sections.push("- `agent-context remember --decision \"rationale\"` — record decisions");
  sections.push("");
...
function buildGeminiMd(systemContent, memorySummary, config) {
  const sections = [
    ...
    "",
    "_Auto-generated by agent-context. Edit `.context/system/` files instead._",
    "_Run `agent-context sync --gemini` to regenerate._",
    "",
  ];
  ...
  sections.push("## Tools");
  sections.push("");
  sections.push("Use `agent-context` CLI for context management:");
  sections.push("```");
  sections.push("agent-context snapshot    # view context tree");
  sections.push("agent-context read <path> # read context file");
  sections.push("agent-context commit <msg> # checkpoint");
  sections.push("agent-context remember --decision \"why\" # record decisions");
  sections.push("```");
  sections.push("");
...
function buildAgentsMd(systemContent, memorySummary, config) {
  const sections = [
    ...
    "",
    "_Auto-generated by agent-context. Edit `.context/system/` files instead._",
    "_Run `agent-context sync --codex` to regenerate._",
    "",
  ];
...
function syncCursorRules(root, ctxDir) {
  ...
  for (const f of files) {
    ...
    const cursorRule = [
      "---",
      `description: "Project ${ruleName} (from agent-context)"`,
      "globs: **/*",
      "alwaysApply: true",
      "---",
      "",
      `_Auto-generated by agent-context from .context/${f.path}_`,
      "",
      content,
      "",
    ].join("\n");

    writeFileSync(join(rulesDir, `actx-${f.name}`), cursorRule);
  }

  console.log(`  📁 Cursor: ${files.length} rules → .cursor/rules/actx-*.md`);
}
...
function syncWindsurf(root, ctxDir, systemContent, memorySummary) {
  ...
  for (const f of files) {
    ...
    writeFileSync(join(rulesDir, `actx-${f.name}`), [
      `_Auto-generated by agent-context from .context/${f.path}_`,
      "",
      content,
      "",
  ...
  console.log(`  📁 Windsurf: ${files.length} rules → .windsurf/rules/actx-*.md`);
}
