...
export default async function snapshot({ args, flags }) {
  ...
  // Branches
  const branchEntries = tree.filter((e) => e.path.startsWith("branches/") && e.isDir && e.depth === 1);
  ...
  // Config summary
  lines.push(`CONFIG: auto_commit=${config.auto_commit} | reflection=${config.reflection?.trigger || "manual"}`);
