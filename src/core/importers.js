...
export function importClaudeHistory(cwd, contextDir) {
  ...
  if (summaries.length) {
    ...
    console.log(
      `  📥 Imported ${totalSessions} Claude Code sessions → memory/imported-claude-sessions.md`,
    );
  }
...
export function importCodexHistory(cwd, contextDir) {
  ...
  if (summaries.length) {
    ...
    console.log(
      `  📥 Imported ${summaries.length} Codex sessions → memory/imported-codex-sessions.md`,
    );
  }
